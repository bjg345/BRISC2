BRISC2_estimation <- function (coords, y, x = NULL, sigma.sq = 1, tau.sq = 0.1, phi = 1,
    nu = 1.5, n.neighbors = 15, n_omp = 1, order = "Sum_coords",
    cov.model = "exponential", search.type = "tree", stabilization = NULL,
    pred.stabilization = 1e-08, verbose = TRUE, eps = 2e-05,
    nugget_status = 1, ordering = NULL, neighbor = NULL, tol = 12)
{
    n <- nrow(coords)
    if (is.null(x)) {
    ...
    p <- ncol(x)
    if (ncol(x) == 1)
        warning("The ordering of inputs x (covariates) and y (response) in BRISC2_estimation has been changed BRISC2 1.0.0 onwards.\nPlease check the new documentation with ?BRISC2_estimation.")
    if (nugget_status == 0) {
        fix_nugget = 0
    }
    if (nugget_status == 1) {
    ...
    if (order == "AMMD" && length(y) < 65) {
        stop("error: Number of data points must be atleast 65 to use AMMD")
    }
    ...
    if (is.matrix(y) & ncol(as.matrix(y)) > 1) {
        stop("error: BRISC2 handles only univariate response. You have used a y (response) with more than one column.\n          The ordering of inputs x (covariates) and y (response) in BRISC2_estimation has been changed BRISC2 0.2.0 onwards.\n          Did you mean x (covaiates) instead of y (response) by the multivariatte input?\n          Please check the new documentation with ?BRISC2_estimation.")
    }
    coords <- round(coords, tol)
    x <- round(x, tol)
    y <- round(y, tol)
    if (tau.sq < 0) {
        stop("error: tau.sq must be non-negative")
    }
    if (sigma.sq < 0) {
        stop("error: sigma.sq must be non-negative")
    }
    if (phi < 0) {
        stop("error: phi must be non-negative")
    }
    if (nu < 0) {
        stop("error: nu must be non-negative")
    }
    if (is.null(ordering)) {
    ...
    X <- x[ord, , drop = FALSE]
    y <- y[ord]
    cov.model.names <- c("exponential", "spherical", "matern",
        "gaussian")
    ...
    if (!search.type %in% search.type.names) {
        stop("error: specified search.type '", search.type, "' is not a valid option; choose from ",
            paste(search.type.names, collapse = ", ", sep = ""),
            ".")
    }
    search.type.indx <- which(search.type == search.type.names) -
        1
    ...
    if (is.null(neighbor)) {
        result <- .Call("BRISC2_estimatecpp", y, X, p, n, n.neighbors,
            coords, cov.model.indx, alpha.sq.starting, sigma.sq.starting,
            phi.starting, nu.starting, search.type.indx, n.omp.threads,
            verbose, eps, fix_nugget, PACKAGE = "BRISC2")
    }
    if (!is.null(neighbor)) {
        ...
        result_short <- .Call("BRISC2_estimateneighborcpp", y,
            X, p, n, n.neighbors, coords, cov.model.indx, alpha.sq.starting,
            sigma.sq.starting, phi.starting, nu.starting, search.type.indx,
            n.omp.threads, verbose, eps, fix_nugget, nnIndxLU,
            CIndx, D, d, nnIndx, j, PACKAGE = "BRISC2")
        result <- append(result_short, neighbor)
    }
    p2 <- proc.time()
    llk <- -(result$log_likelihood + n * log(2 * pi))/2
    result_list <- list()
