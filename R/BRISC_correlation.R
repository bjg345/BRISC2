BRISC2_correlation <- function(coords, sim, sigma.sq = 1, tau.sq = 0, phi = 1, nu = 1.5, n.neighbors = NULL, n_omp = 1,
  ...
  if(isTRUE(stabilization)){
    tau.sq <- max(tau.sq, 1e-06 * sigma.sq)
  ...
  ##estimtion
  result <- .Call("BRISC2_correlationcpp", n, n.neighbors, coords, cov.model.indx, alpha.sq.starting, phi.starting, nu.starting, search.type.indx, n.omp.threads, verbose, sim, sim_number, fix_nugget, PACKAGE = "BRISC2")
