BRISC2_prediction <- function(BRISC2_Out, coords.0, X.0 = NULL, n_omp = 1, verbose = TRUE, tol = 12){


  if(missing(BRISC2_Out)){stop("error: BRISC2_prediction expects BRISC2_Out\n")}
  ...
  coords <- BRISC2_Out$coords

  n.omp.threads <- as.integer(n_omp)
  n <- nrow(X)
  p <- ncol(X)
  Theta <- as.matrix(BRISC2_Out$Theta)
  dim(Theta) <- c(length(BRISC2_Out$Theta),1)
  Beta <- as.matrix(BRISC2_Out$Beta)
  dim(Beta) <- c(length(BRISC2_Out$Beta),1)
  ...
  out <- .Call("BRISC2_predictioncpp", X, y, coords, n, p, n.neighbors, X.0, coords.0, q, nn.indx.0,
               Beta, Theta, cov.model.indx, n.omp.threads, verbose, PACKAGE = "BRISC2")
  p6 <- proc.time()
